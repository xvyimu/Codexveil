...
function Get-DreamSkinThemePaths {
  ...
  return [pscustomobject]@{
    ...
    ThemesLock = Join-Path $fullRoot 'themes\themes.locked'
  }
}

function Test-DreamSkinThemesLocked {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'))
  $paths = Get-DreamSkinThemePaths -StateRoot $StateRoot
  return (Test-Path -LiteralPath $paths.ThemesLock -PathType Leaf)
}

function Assert-DreamSkinThemesUnlocked {
  param([string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'CodexDreamSkin'))
  if (Test-DreamSkinThemesLocked -StateRoot $StateRoot) {
    throw 'Single-skin lock is on (themes.locked). Unlock first: lock-themes.ps1 -Unlock'
  }
}
...
function Initialize-DreamSkinThemeStore {
  ...
  # Single-skin lock: when themes.locked exists, do not reseed bundled presets into
  # the saved-theme catalog. This keeps F6 on active-theme only and avoids fighting
  # the themes directory ACL lock used by the taskbar launcher.
  if (-not (Test-DreamSkinThemesLocked -StateRoot $StateRoot)) {
...
function Save-DreamSkinCurrentTheme {
  ...
  $paths = Get-DreamSkinThemePaths -StateRoot $StateRoot
  Assert-DreamSkinThemesUnlocked -StateRoot $StateRoot
  Ensure-DreamSkinManagedDirectory -Path $paths.Root -Root $paths.Root
