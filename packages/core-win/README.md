...
| 文件 | LOC | 关键函数前缀 | 说明 |
|---|---|---|---|
| `launcher-ui.ps1` | 850 | `Get-CodexSkinProgramRoot`, `Get-CodexSkin*Log`, `Show-CodexSkinBalloon`, `Ensure-CodexSkinTray`, `Focus-CodexSkinWindow`, `Get-CodexSkinRuntime*` | 启动器公共 UI：日志/气泡/托盘/焦点/runtime 解析 |
| `common-windows.ps1` | 644 | `Get-DreamSkinCdp*`, `Get-DreamSkinCodex*`, `Read/Write-DreamSkinState`, `Stop-DreamSkin*` | Codex 发现 · 端口/CDP · state.json · 进程 |
...
若 `launcher-ui.ps1` 850 行确实成为维护负担，考虑按 `Get-* / Show-* / Focus-* / Ensure-*` 前缀切分。
先不动，因为：
