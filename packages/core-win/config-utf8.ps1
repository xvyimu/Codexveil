...
function Restore-DreamSkinBaseTheme {
  ...
  # Replacing a generated final setting can leave one separator behind.  When
  # the remaining body is otherwise byte-for-byte equivalent in content to the
  # backed-up table, retain the backup's exact terminal newline convention.
  # Do not normalize a body that contains user edits made after installation.
  if ($null -ne $backupDesktop -and
    $body.TrimEnd("`r", "`n") -ceq $backupDesktop.Body.TrimEnd("`r", "`n")) {
