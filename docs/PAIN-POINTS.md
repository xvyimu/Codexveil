...
| 25 | 窗内 **F6** 循环换肤 / toast 不可用 | 1.3.25 | **代码已恢复；需 publish 到安装态**（2026-07-22）· 仓内 `renderer-inject` 4 参 catalog + `cycleTheme`/`setTheme` + F6/toast；安装态 versions 仍旧直到 `publish-runtime.ps1`。F6 不写 active-theme（瞬切）；持久换肤仍托盘/面板/CLI |
...
| F6 | **中（需 publish）** | #25 仓内已恢复；安装态需 publish；瞬切不写盘 |
