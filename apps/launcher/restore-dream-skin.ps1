...
try {
  ...
  if (Test-Path -LiteralPath $ProgramRoot) {
    $installedRuntime = Resolve-DreamSkinCurrentRuntime -ProgramRoot $ProgramRoot
  }
  ...
  try {
    ...
    if ($null -ne $installedRuntime) { $trayScriptsToStop += $installedRuntime.TrayScript }
    ...
    if ($Uninstall) {
      ...
      Remove-DreamSkinRuntimeInstallation -ProgramRoot $ProgramRoot
    }
